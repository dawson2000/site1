...
if ($num_products_count > 0) {
  ...
  while (!$new_products->EOF) {
	...
	$products_desc = substr(strip_tags($new_products->fields['products_description']), 0, 15) . '...';
	
    $products_price = '<strong>' . zen_get_products_display_price($new_products->fields['products_id']) . '</strong>';
	
	$products_butt = '<a href="' . zen_href_link(zen_get_info_page($new_products->fields['products_id']), 'cPath=' . $productsInCategory[$new_products->fields['products_id']] . '&products_id=' . $new_products->fields['products_id']) . '">' . zen_image_button(BUTTON_IMAGE_GOTO_PROD_DETAILS, BUTTON_GOTO_PROD_DETAILS_ALT) . '</a>';
	...
	if (SHOW_PRODUCT_INFO_COLUMNS_NEW_PRODUCTS > 1 && $num_products_count > 1) {
		...
		if ($col > 1 && $col < SHOW_PRODUCT_INFO_COLUMNS_NEW_PRODUCTS) {
			$tm_param = 'style="margin-left:20px;"';
			
		} elseif ($col > 0 && $col < SHOW_PRODUCT_INFO_COLUMNS_NEW_PRODUCTS){
			$tm_param = 'style="margin-left:10px; margin-right:10px;"';
		}			
		 else {
			$tm_param = '';
		}
